...
namespace RenderEngine.Screens
{
	...
	public class OnScreenKeyboard
	{
        ...
        /// </summary>
        public OnScreenKeyboard(ScreenManager Manager)			
		{
			...
			touchKeys.Add(Keys.Back, new TouchKey(10, "⇦Back", "⇦Back", "", Keys.Back) { width = 2, fontScale = .67f, repeat = true });
			...
			touchKeys.Add(Keys.Enter, new TouchKey(20, "Ret", "Ret", "", Keys.Enter) { width = 2, fontScale = .67f });
            ...
            touchKeys.Add(Keys.OemQuestion, new TouchKey(28, "?", "?", "", Keys.OemQuestion));
            
            touchKeys.Add(Keys.RightShift, new TouchKey(29, "⇧Shift", "⇧Shift", "", Keys.RightShift) { offset = 1.5f, width = 1.5f, fontScale = .67f });

			//Row 4
			touchKeys.Add(Keys.Space, new TouchKey(30, "Space", "", "", Keys.Space) { offset = 3, width = 5 });
			touchKeys.Add(Keys.Escape, new TouchKey(31, "Esc⇩", "Esc⇩", "", Keys.Escape) { offset = 3, fontScale = .5f });
		...
		protected void SetKeyboardDimensions()
		{
            ...
            keyboardWidth = Manager.InternalWidth;
			keyboardHeight = Manager.InternalHeight * 40 / 100;
			keyWidth = keyboardWidth / 12;
			keyHeight = keyboardHeight / 4;

			//The starting point to draw the keys from
			Vector2 pointer = new Vector2(0, Manager.InternalHeight - keyboardHeight);
		...
		public  void LoadContent()
		{
			...
			kbTexture = new Texture2D(Manager.GraphicsDevice, 1, 1);
			kbTexture.SetData(new Color[] { Color.White });

			//Attempt to load the font from the root content directory
			try
			{
				Font = Manager.content.Load<SpriteFont>("Segoe UI Symbol");
			}
			catch (Exception ex)
			{
		...
		public void Draw(GameTime gameTime)
		{
			...
			Rectangle keyboardDrawBox = new Rectangle(0, Manager.InternalHeight - keyboardHeight, keyboardWidth, keyboardHeight);
			...
			foreach (TouchKey key in touchKeys.Values)
			{
                ...
                {
                  //  Console.WriteLine("KeyboardDraw:" + ex.Message + ex.StackTrace);
                }
